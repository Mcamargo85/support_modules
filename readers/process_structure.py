...
def load_process_structure(bpmn, verbose):
    ...
    # Adding nodes
    index = create_nodes(g, total_elements, 0, list(filter(lambda x: x['task_name'] == 'Start', tasks)), 'start',
                         'task_name', 'task_id', verbose)
    index = create_nodes(g, total_elements, index,
                         list(filter(lambda x: x['task_name'] not in ['Start', 'End'], tasks)), 'task', 'task_name',
                         'task_id', verbose)
    index = create_nodes(g, total_elements, index, list(filter(lambda x: x['task_name'] == 'End', tasks)), 'end',
                         'task_name', 'task_id', verbose)
    index = create_nodes(g, total_elements, index, list(filter(lambda x: x['gate_dir'] == 'Diverging', ex_gates)),
                         'gate', 'gate_name', 'gate_id', verbose)
    index = create_nodes(g, total_elements, index, list(filter(lambda x: x['gate_dir'] == 'Converging', ex_gates)),
